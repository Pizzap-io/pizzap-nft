...
abstract contract PizzapNFTRandomizer {
    function getTokenId(uint256 tokenId) public view virtual returns(string memory);
}

contract PizzapNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    event PizzapNFTRevealed(uint256 tokenId, string fileId); // Sending the event for offchain script to transform the right file
    ...
    function mintTransfer(address to) public returns(uint256) {
        ...
        PizzapNFTRandomizer tokenAttribution = PizzapNFTRandomizer(randomizerAddress);
        
        string memory realId = tokenAttribution.getTokenId(_tokenIdCounter.current());
        uint256 mintedId =  _tokenIdCounter.current();
        
        _safeMint(to, _tokenIdCounter.current());
        emit PizzapNFTRevealed(_tokenIdCounter.current(), realId);
