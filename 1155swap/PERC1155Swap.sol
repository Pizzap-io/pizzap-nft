...
contract PERC1155Swap is ERC1155Holder, OwnableUpgradeable {
    ...
    modifier canBuy(uint256 _orderId, uint256 _quantity) {
    ...
    function _putOn(
        ...
        IERC1155 nftToken = IERC1155(_tokenAddr);
    ...
    function buyByMapi(uint256 _orderId, uint256 _quantity)
        external
        canBuy(_orderId, _quantity)
        returns (uint256)
    {
    ...
    function buyByUsdt(uint256 _orderId, uint256 _quantity)
        external
        canBuy(_orderId, _quantity)
        returns (uint256)
    {
    ...
    function buy(
        ...
        require(
            transferNftTo(_msgSender(), _orderId, _quantity) == true,
            "nft transfer fail"
        );
        ...
        returns (uint256)
    {
        ...
        if (allItems[_orderId].quantity > 0) {
            ...
            left = item.quantity;
            IERC1155 nftToken = IERC1155(nft.contractAddress);
            nftToken.safeTransferFrom(
                address(this),
                item.seller,
                nft.tokenId,
